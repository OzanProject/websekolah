                    ...
                    @foreach ($facilities as $index => $item)
                        <tr id="row-{{ $item->id }}">
                            ...
                            <td class="text-center">
                                ...
                                <form action="{{ route('admin.facilities.destroy', $item->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus fasilitas ini?')">
                                    @csrf
                                    @method('DELETE')
    ...
    <div class="modal fade" id="importModal" tabindex="-1" role="dialog" aria-labelledby="importModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form action="{{ route('admin.facilities.import') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-header bg-success text-white">
                        <h5 class="modal-title" id="importModalLabel">Import Data Fasilitas</h5>
                    ...
                    <div class="modal-body">
                        ...
                        <a href="{{ route('admin.facilities.template') }}" class="btn btn-outline-success mb-4">
    ...
    <script>
        $(document).ready(function() {
            var table = $('#facilitiesTable').DataTable({
                ...
                "responsive": true,
                "columnDefs": [
                    { "orderable": false, "targets": [0, 2, 6] } // Disable ordering on checkbox, image, and action columns
                ],
                "language": {
            ...
            $('#facilitiesTable tbody').on('change', '.checkItem', function() {
                ...
                if (selectedIds.length > 0) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                url: "{{ route('admin.facilities.bulkDestroy') }}",
                                type: 'POST',
                                data: {
                                    _token: "{{ csrf_token() }}",
                                    ids: selectedIds
