...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <table id="galleriesTable" class="table table-bordered table-striped table-hover">
                <thead>
                    <tr>
                        ...
                        <th style="width: 150px">Preview Gambar</th>
                ...
                <tbody>
                    @foreach ($galleries as $index => $item)
                        <tr id="row-{{ $item->id }}">
                            ...
                            <td>
                                @if($item->image_path)
                                    <img src="{{ filter_var($item->image_path, FILTER_VALIDATE_URL) ? $item->image_path : Storage::url($item->image_path) }}" alt="{{ $item->title }}" class="img-thumbnail" style="max-height: 80px; width: auto;">
                                @else
    ...
    <div class="modal fade" id="importModal" tabindex="-1" role="dialog" aria-labelledby="importModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form action="{{ route('admin.galleries.import') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-header bg-success text-white">
                        <h5 class="modal-title" id="importModalLabel">Import Data Galeri</h5>
                        <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body">
                        <p>Silakan unduh template Excel berikut, isi judul foto Anda, lalu unggah kembali ke sini.</p>
                        <a href="{{ route('admin.galleries.template') }}" class="btn btn-outline-success mb-4">
    ...
    <script>
        $(document).ready(function() {
            var table = $('#galleriesTable').DataTable({
                ...
                "responsive": true,
                "columnDefs": [
                    { "orderable": false, "targets": [0, 2, 4] } // Disable ordering on checkbox, image, and action columns
                ],
                "language": {
            ...
            $('#galleriesTable tbody').on('change', '.checkItem', function() {
                ...
                if (selectedIds.length > 0) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                url: "{{ route('admin.galleries.bulkDestroy') }}",
                                type: 'POST',
                                data: {
                                    _token: "{{ csrf_token() }}",
                                    ids: selectedIds
