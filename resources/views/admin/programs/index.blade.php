                    ...
                    @foreach ($programs as $index => $program)
                        <tr id="row-{{ $program->id }}">
                            ...
                            <td class="text-center">
                                ...
                                <form action="{{ route('admin.programs.destroy', $program->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus program ini?')">
                                    @csrf
                                    @method('DELETE')
            ...
            <div class="modal-content">
                <form action="{{ route('admin.programs.import') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-header bg-success text-white">
                        <h5 class="modal-title" id="importModalLabel">Import Data Program</h5>
                    ...
                    <div class="modal-body">
                        ...
                        <a href="{{ route('admin.programs.template') }}" class="btn btn-outline-success mb-4">
...
@section('js')
    <script>
        $(document).ready(function() {
            ...
            $('#programsTable tbody').on('change', '.checkItem', function() {
                ...
                if (selectedIds.length > 0) {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                url: "{{ route('admin.programs.bulkDestroy') }}",
                                type: 'POST',
                                data: {
                                    _token: "{{ csrf_token() }}",
                                    ids: selectedIds
